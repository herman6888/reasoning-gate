...
![reasoning-gate 拓扑](topology_cn.png)

```
client (Hermes chat_completions / Codex responses)
        │
        ▼
   reasoning-gate :8910  ──► Kev-4B 决策服务 (/v1/systemone)
...
整夜窗口（00:14 → 08:00）：65 次经代理请求——真决策 13、lease 命中 45（69% 零决策
开销）、fail-open 7。决策延迟中位 2.8s，最快 0.56s，GPU 争用时最慢 9.3s。

墙钟节省来自更低 effort 产生更短的 thinking 块；输出 token 节省较小，因为最终答案
长度受任务本身约束。最大收益出现在 Kev 高置信判为简单的任务上（Strix Halo：墙钟 −75%）。
